...
func (r *ResourcePostgreSQL) migrate(ctx context.Context, plan, state PostgreSQL, diags *diag.Diagnostics) {
	addonsProvidersRes := tmp.GetAddonsProviders(ctx, r.Client())
	if addonsProvidersRes.HasError() {
	...
	for _, migration := range *migrations {
		if migration.Status == "RUNNING" {
			diags.AddError(
				...
				fmt.Sprintf("A migration (ID: %s) is already running for this addon. Please wait for it to complete before requesting a new migration.", migration.MigrationID),
			)
			return
		}
	}
	...
	migrationReq := tmp.AddonMigrationRequest{
	...
	for {
		...
		select {
		case <-ctx.Done():
			diags.AddError("migration timeout", "Migration did not complete within the allowed time")
			return
		case <-t.C:
			...
			if currentMigration.Status == "OK" {
				...
				// Migration failed or in an unexpected state
				diags.AddError(
					"migration failed",
