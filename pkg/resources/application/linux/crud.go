...
func (r *ResourceLinux) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	tflog.Debug(ctx, "ResourceLinux.Create()")

	plan := helper.PlanFrom[Linux](ctx, req.Plan, &resp.Diagnostics)
...
func (r *ResourceLinux) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	tflog.Debug(ctx, "ResourceLinux.Read()")

	state := helper.StateFrom[Linux](ctx, req.State, &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
...
// Update resource
func (r *ResourceLinux) Update(ctx context.Context, req resource.UpdateRequest, res *resource.UpdateResponse) {
	tflog.Debug(ctx, "ResourceLinux.Update()")

	// Retrieve values from plan and state
	plan := helper.PlanFrom[Linux](ctx, req.Plan, &res.Diagnostics)
	if res.Diagnostics.HasError() {
		return
	}
	state := helper.StateFrom[Linux](ctx, req.State, &res.Diagnostics)
	if res.Diagnostics.HasError() {
		return
	}
